# app.py (예측 테이블 .style.format 오류 수정)
...
# --- 사이드바 설정 ---
with st.sidebar:
    # ... (이전과 동일한 사이드바 설정: 티커, 기간, 분기 수, 평단가, 수량, 버튼) ...
    st.header("⚙️ 분석 설정")
    ...
    num_trend_periods_input = st.number_input("영업이익률 추세 분기 수", min_value=2, max_value=12, value=4, step=1)
        ...
        with st.spinner(f"{ticker_processed} 종목 분석 중..."):
            try:
                ...
                if results and isinstance(results, dict):
                    ...
                    # 3. 영업이익률 추세
                    # ... (이전과 동일) ...
                    st.subheader(f"📈 영업이익률 추세 (최근 {num_trend_periods_input} 분기)")
                    ...
                    elif margin_trend_data is None: st.info("영업이익률 추세 데이터 없음.")
                    else: st.info("표시할 영업이익률 추세 데이터 없음.")
                    st.divider()
                    ...
                    if isinstance(forecast_data_list, list):
                        ...
                        # .style.format()에 딕셔너리를 사용하여 숫자 컬럼만 지정
                        st.dataframe(
                            df_forecast_display[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].tail(10).style.format({
                                'yhat': "{:.2f}",
                                'yhat_lower': "{:.2f}",
                                'yhat_upper': "{:.2f}"
                                # 'ds' 컬럼은 여기서 지정하지 않음 (문자열이므로)
                            }),
                    ...
                    if avg_price > 0 and isinstance(forecast_data_list, list):
                        ...
                        try:
                            ...
                            st.markdown("##### 리스크 요약"); col_r1, col_r2, col_r3 = st.columns(3); col_r1.metric("⚠️ 예측 하한가 < 평단가 일수", f"{risk_days} 일 / {forecast_days} 일"); col_r2.metric("📉 최대 예상 손실률", f"{max_loss_pct:.2f}%")
                            ...
                            fig_risk.add_trace(go.Scatter(x=df_pred['ds'], y=pd.to_numeric(df_pred['yhat_upper'], errors='coerce'), mode='lines', line_color='rgba(0,100,80,0.2)', name='Upper Bound')); fig_risk.add_trace(go.Scatter(x=df_pred['ds'], y=df_pred['yhat_lower'], mode='lines', line_color='rgba(0,100,80,0.2)', name='Lower Bound', fill='tonexty', fillcolor='rgba(0,100,80,0.1)')); fig_risk.add_trace(go.Scatter(x=df_pred['ds'], y=pd.to_numeric(df_pred['yhat'], errors='coerce'), mode='lines', line=dict(dash='dash', color='rgba(0,100,80,0.6)'), name='Forecast (yhat)')); fig_risk.add_hline(y=avg_price, line_dash="dot", line_color="red", annotation_text=f"평단가: ${avg_price:.2f}", annotation_position="bottom right"); df_risk_periods = df_pred[df_pred['리스크 여부']]
                            ...
                            fig_risk.update_layout(title=f"{ticker_processed} 예측 구간 vs 평단가 비교", xaxis_title="날짜", yaxis_title="가격", hovermode="x unified", margin=dict(l=20, r=20, t=40, b=20)); st.plotly_chart(fig_risk, use_container_width=True)
                            if risk_days > 0:
