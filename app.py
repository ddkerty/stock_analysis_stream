# app.py (뉴스 목록 표시 SyntaxError 최종 수정 버전)
...
try:
    ...
    st.stop()
     ...
     st.stop()
except Exception as import_err: # SyntaxError 등 다른 import 오류 처리
...
# Secrets 또는 .env에서 키 로드 시도
if secrets_available:
    try:
        NEWS_API_KEY = st.secrets["NEWS_API_KEY"]
        FRED_API_KEY = st.secrets["FRED_API_KEY"]
        if NEWS_API_KEY and FRED_API_KEY: api_keys_loaded = True
        else: sidebar_status.error("Secrets에 API 키 값이 비어있습니다.")
    except KeyError:
         sidebar_status.warning("Secrets API 키 없음. Cloud 설정 확인.")
    except Exception as e:
        sidebar_status.error(f"Secrets 로드 오류: {e}")

if not api_keys_loaded and not secrets_available: # 로컬 환경 등에서만 .env 시도
...
if not api_keys_loaded:
    st.sidebar.error("API 키 로드 실패! 기능 제한됨.")
    # 앱 실행을 막도록 주석 해제 가능
    # st.stop()
...
def run_cached_analysis(ticker, news_key, fred_key, years, days, num_trend_periods):
    ...
    if not news_key or not fred_key:
         logging.error("API 키가 유효하지 않습니다 (캐시 함수).")
         return {"error": "API 키 미유효"}
    try:
        analysis_result = sa.analyze_stock(ticker, news_key, fred_key, analysis_period_years=years, forecast_days=days, num_trend_periods=num_trend_periods)
        if analysis_result is None:
             return {"error": "분석 중 초기 오류 발생"}
        return analysis_result
    except Exception as e:
        logging.error(f"분석 함수 실행 중 예외 발생: {e}")
        logging.error(traceback.format_exc())
        return {"error": f"분석 중 예기치 않은 오류 발생: {e}"}
...
if analyze_button:
    if not ticker_input:
        ...
        results_placeholder.error("API 키가 로드되지 않아 분석 불가.")
    else:
                ...
                if results and isinstance(results, dict) and "error" not in results:
                        ...
                        if margin_trend_data and isinstance(margin_trend_data, list):
                            try:
                                ...
                                with st.expander("데이터 보기"):
                                    st.dataframe(df_margin.style.format("{:.2f}%"), use_container_width=True)
                            except Exception as e:
                                st.error(f"영업이익률 처리 오류: {e}")
                        else:
                    ...
                    with col_news:
                        ...
                        if isinstance(news_sentiment, list) and len(news_sentiment) > 0:
                            st.info(news_sentiment[0]) # 평균 또는 에러 표시
                            # --- ⭐️ SyntaxError 최종 수정: for 반복문 사용 ---
                            with st.expander("최근 뉴스 목록 보기", expanded=False):
                                for line in news_sentiment[1:]:
                                    st.write(f"- {line}")
                            # --------------------------------------------
                        else: # news_sentiment가 리스트가 아니거나 빈 경우
                    ...
                    st.dataframe(df_fcst[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].tail(10).style.format({'yhat': "{:.2f}", 'yhat_lower': "{:.2f}", 'yhat_upper': "{:.2f}"}), use_container_width=True) # 수정된 포맷팅
                    cv_plot_path = results.get('cv_plot_path')
                    if cv_plot_path and isinstance(cv_plot_path, str) and os.path.exists(cv_plot_path): st.markdown("**📉 교차 검증 결과 (MAPE)**"); st.image(cv_plot_path, caption="MAPE (낮을수록 좋음)")
                    elif cv_plot_path is None and results.get('data_points', 0) > 30: st.caption("교차 검증(CV) 오류 또는 데이터 부족.")
                    st.divider()
                    ...
                    if isinstance(forecast_data_list, list) and len(forecast_data_list) > 0:
                        try:
                            ...
                            summary.append(f"- 예측: 향후 {forecast_days}일간 **{trend_obs}** 추세 (${forecast_data_list[-1]['yhat_lower']:.2f} ~ ${forecast_data_list[-1]['yhat_upper']:.2f}).")
                        except:
                            summary.append("- 예측 요약 오류.")
                    if isinstance(news_sentiment, list) and len(news_sentiment) > 0:
                        try:
                            ...
                            summary.append(f"- 뉴스: 평균 {avg_score:.2f}, **{sentiment_desc}** 분위기.")
                        except:
                            summary.append("- 뉴스 요약 오류.")
                    if isinstance(fng_index, dict):
                        summary.append(f"- 심리: F&G {fng_index.get('value', 'N/A')} (**{fng_index.get('classification', 'N/A')}**).")
                    if fundamentals and isinstance(fundamentals, dict):
                        per_val = fundamentals.get("PER", "N/A")
                        sector_val = fundamentals.get("업종", "N/A")
                        fund_summary = []
                        if per_val != "N/A":
                            fund_summary.append(f"PER {per_val}")
                        if sector_val != "N/A":
                            fund_summary.append(f"'{sector_val}'")
                        if fund_summary:
                            summary.append(f"- 정보: {', '.join(fund_summary)}.")
                    trend_summary = []
                    margin_trend_data = results.get('operating_margin_trend')
                    roe_trend_data = results.get('roe_trend')
                    debt_trend_data = results.get('debt_to_equity_trend')
                    current_trend_data = results.get('current_ratio_trend')
                    # 각 추세 데이터 마지막 값 접근 전 리스트 비어있는지 확인 추가
                    if margin_trend_data and isinstance(margin_trend_data, list):
                        trend_summary.append(f"영업익률 {margin_trend_data[-1]['Operating Margin (%)']:.2f}%")
                    if roe_trend_data and isinstance(roe_trend_data, list):
                        trend_summary.append(f"ROE {roe_trend_data[-1]['ROE (%)']:.2f}%")
                    if debt_trend_data and isinstance(debt_trend_data, list):
                        trend_summary.append(f"부채비율 {debt_trend_data[-1]['D/E Ratio']:.2f}")
                    if current_trend_data and isinstance(current_trend_data, list):
                        trend_summary.append(f"유동비율 {current_trend_data[-1]['Current Ratio']:.2f}")
                    if trend_summary:
                        summary.append(f"- 최근 재무: {', '.join(trend_summary)}.")
                    if avg_price > 0 and isinstance(forecast_data_list, list) and 'risk_days' in locals():
                        if risk_days > 0:
                            summary.append(f"- 리스크: **{risk_days}일** 평단가 하회 가능성 (Max 손실률 {max_loss_pct:.2f}%).")
                        else:
                            summary.append(f"- 리스크: 예측상 평단가 하회 가능성 없음.")
                    if summary:
                        st.markdown("\n".join(summary))
                        st.caption("⚠️ **주의:** 투자 조언 아님. 모든 결정은 본인 책임.")
                    else:
