# -*- coding: utf-8 -*-
# Combined app.py V1.8 - Comprehensive Analysis + Technical Analysis (VWAP, BB, Fib)
...
def plot_technical_chart(df, ticker):
    ...
    elif 'Upper' in df.columns: st.caption("볼린저 밴드 데이터 부족/오류.")
...
# --- Streamlit 페이지 설정 ---
st.set_page_config(page_title="종합 주식 분석 V1.8", layout="wide", initial_sidebar_state="expanded")
...
# --- 사이드바 설정 ---
with st.sidebar:
    st.image("https://cdn-icons-png.flaticon.com/512/10071/10071119.png", width=80) # 예시 로고
    st.title("📊 분석 도구 V1.8")
    st.markdown("---")
                ...
                try:
                    ...
                    if results and isinstance(results, dict) and "error" not in results:
                             ...
                             try:
                                 ...
                                 if not all(col in df_pred.columns for col in required_fcst_cols): st.warning("예측 컬럼 부족.")
                                     ...
                                     if not df_pred.empty:
                                         ...
                                         col_r1.metric("⚠️ < 평단가 예측 일수", f"{risk_days}일 / {days}일"); col_r2.metric("📉 Max 예측 손실률", f"{max_loss_pct:.2f}%")
                                         if qty > 0: col_r3.metric("💸 Max 예측 손실액", f"${max_loss_amt:,.2f}")
                                         if risk_days > 0: st.warning(f"{days}일 예측 중 **{risk_days}일** 평단가(${avg_p:.2f}) 하회 가능성 (Max 손실률: **{max_loss_pct:.2f}%**).")
                                         ...
                                         fig_risk.update_layout(title=f"{ticker_proc} 예측 구간 vs 평단가 비교", xaxis_title="날짜", yaxis_title="가격", hovermode="x unified", margin=dict(l=20, r=20, t=40, b=20)); st.plotly_chart(fig_risk, use_container_width=True)
                                         if risk_days > 0:
                        ...
                        if isinstance(forecast_data_list, list) and len(forecast_data_list) > 0: # 예측
                            try:
                                start_pred_row = forecast_data_list[0]; end_pred_row = forecast_data_list[-1]; start_pred = pd.to_numeric(start_pred_row.get('yhat'), 'coerce'); end_pred = pd.to_numeric(end_pred_row.get('yhat'), 'coerce')
                                ...
                                if pd.notna(start_pred) and pd.notna(end_pred):
                                    trend_obs = "상승" if end_pred > start_pred else "하락" if end_pred < start_pred else "횡보"; lower_str = f"${lower:.2f}" if pd.notna(lower) else 'N/A'; upper_str = f"${upper:.2f}" if pd.notna(upper) else 'N/A'
                                    summary_points.append(f"- **예측:** 향후 {days}일간 **{trend_obs}** 추세 ({lower_str}~{upper_str}).")
                                else: summary_points.append("- 예측: 값 유효하지 않음.")
                            except Exception as e: logging.warning(f"예측 요약 오류: {e}"); summary_points.append("- 예측: 요약 오류.")
                        if isinstance(results.get('news_sentiment'), list) and len(results['news_sentiment']) > 0 and ":" in results['news_sentiment'][0]: # 뉴스
                            try:
                                score_part = results['news_sentiment'][0].split(":")[-1].strip(); avg_score = float(score_part); sentiment_desc = "긍정적" if avg_score > 0.05 else "부정적" if avg_score < -0.05 else "중립적"
                        ...
                        if isinstance(results.get('fear_greed_index'), dict): fng = results['fear_greed_index']; summary_points.append(f"- **시장 심리:** 공포-탐욕 {fng.get('value', 'N/A')} (**{fng.get('classification', 'N/A')}**).") # F&G
                        ...
                        trend_parts = [] # 재무 추세
                        try:
                            if results.get('operating_margin_trend') and results['operating_margin_trend']: trend_parts.append(f"영업익률 {results['operating_margin_trend'][-1].get('Op Margin (%)', 'N/A'):.2f}%")
...
# ============== 📈 기술 분석 탭 ==============
    ...
    bb_window_val = st.session_state.get('bb_window', 20) # 사이드바 설정값 가져오기
    bb_std_val = st.session_state.get('bb_std', 2.0) # 사이드바 설정값 가져오기
    ...
    if analyze_button_tech:
        if not ticker_tech: st.warning("종목 티커를 입력해주세요.")
            ...
            with st.spinner(f"{ticker_processed_tech} 데이터 로딩 및 지표 계산 중..."):
                try: # yfinance 다운로드
            ...
            if not df_tech.empty:
                ...
                required_cols = ['Open', 'High', 'Low', 'Close', 'Volume'] # VWAP, BB 계산 전 필요한 기본 컬럼
                if not all(col in df_tech.columns for col in required_cols):
                    st.error(f"❌ 기본 컬럼({required_cols}) 누락. 실제: {df_tech.columns.tolist()}")
                    ...
                    try: # 지표 계산 및 차트 생성
                        ...
                        if df_tech.empty: st.warning("데이터 정제 후 남은 데이터 없음.")
                            ...
                            except ValueError as e: st.warning(f"VWAP 계산 불가: {e}")
                            # 볼린저 밴드 계산
                            try: df_tech = calculate_bollinger_bands(df_tech, window=bb_window_val, num_std=bb_std_val)
                            except ValueError as e: st.warning(f"볼린저 밴드 계산 불가: {e}")
                            ...
                            display_cols = [col for col in display_cols if col in df_tech.columns] # 존재하는 컬럼만 선택
                         ...
                         st.error(f"기술적 분석 처리 중 오류: {e}"); logging.error(f"Tech analysis error: {traceback.format_exc()}"); st.dataframe(df_tech.tail(10), use_container_width=True)
            elif analyze_button_tech: st.error(f"❌ 데이터 로드 실패. 티커/기간/간격 확인 필요.")
    else: st.info("종목 티커, 기간, 간격 설정 후 '기술적 분석 실행' 버튼 클릭.")
