# -*- coding: utf-8 -*-
# Combined app.py V1.9.5 - Fixed Second SyntaxError
...
# --- 기술 분석 함수 ---
# (calculate_vwap, calculate_bollinger_bands, plot_technical_chart 함수는 V1.9.4와 동일)
...
# --- Streamlit 페이지 설정 ---
st.set_page_config(page_title="종합 주식 분석 V1.9.5", layout="wide", initial_sidebar_state="expanded") # 버전 업데이트
                            ...
                            if isinstance(news_sentiment, list) and len(news_sentiment) > 0:
                                ...
                                if len(news_sentiment) > 1:
                                    with st.expander("뉴스 목록 보기"):
                                        for line in news_sentiment[1:]: # 일반 for문 사용
                                            st.write(f"- {line}")
                                # --- 수정 끝 ---
                    ...
                    try:
                        ...
                        if df_processed.empty: st.warning("데이터 정제 후 남은 데이터가 없습니다.")
                            ...
                            if not df_calculated.empty:
                                latest_row = df_calculated.iloc[-1]; signal_messages = []
...
# --- 앱 정보 ---
st.sidebar.markdown("---")
st.sidebar.info("종합 주식 분석 툴 V1.9.5 | 정보 제공 목적 (투자 조언 아님)")