# -*- coding: utf-8 -*-
# Combined app.py V1.9.4 - Fixed SyntaxError in Comprehensive Analysis Tab
...
# --- 기술 분석 함수 ---
# (calculate_vwap, calculate_bollinger_bands, plot_technical_chart 함수는 V1.9.3과 동일)
def calculate_vwap(df):
    df = df.copy(); required_cols = ['High', 'Low', 'Close', 'Volume'] # 세미콜론 사용은 문법 오류는 아니지만, 가독성을 위해 줄바꿈 권장
    missing_cols = [col for col in required_cols if col not in df.columns]
    if missing_cols: raise ValueError(f"VWAP 계산 실패: 컬럼 부족 ({missing_cols})")
    if df['Volume'].isnull().all() or df['Volume'].sum() == 0: df['VWAP'] = np.nan; logging.warning(f"Ticker {df.attrs.get('ticker', '')}: VWAP 계산 불가 (거래량 부족/0)") # 세미콜론 사용
    else:
        df['Volume'].fillna(0, inplace=True); df['typical_price'] = (df['High'] + df['Low'] + df['Close']) / 3 # 세미콜론 사용
        df['tp_volume'] = df['typical_price'] * df['Volume']; df['cumulative_volume'] = df['Volume'].cumsum() # 세미콜론 사용
        df['cumulative_tp_volume'] = df['tp_volume'].cumsum()
        df['VWAP'] = np.where(df['cumulative_volume'] > 0, df['cumulative_tp_volume'] / df['cumulative_volume'], np.nan)
    return df

def calculate_bollinger_bands(df, window=20, num_std=2):
    df = df.copy(); required_col = 'Close' # 세미콜론 사용
    ...
    if len(valid_close) < window:
        st.warning(f"BB 계산 위한 유효 데이터({len(valid_close)}개)가 기간({window}개)보다 부족."); df['MA20'] = np.nan; df['Upper'] = np.nan; df['Lower'] = np.nan # 세미콜론 사용
    else:
        ...
        df['Upper'] = df['MA20'] + num_std * df['STD20']; df['Lower'] = df['MA20'] - num_std * df['STD20'] # 세미콜론 사용
    return df

def plot_technical_chart(df, ticker):
    fig = go.Figure(); required_candle_cols = ['Open', 'High', 'Low', 'Close'] # 세미콜론 사용
    if not all(col in df.columns for col in required_candle_cols) or df[required_candle_cols].isnull().all(axis=None): st.error(f"캔들차트 필요 컬럼({required_candle_cols}) 없음/데이터 없음."); return fig # 세미콜론 사용
    ...
    if not valid_price_df.empty:
        min_price = valid_price_df['Low'].min(); max_price = valid_price_df['High'].max(); diff = max_price - min_price # 세미콜론 사용
        if diff > 0:
...
# --- Streamlit 페이지 설정 ---
st.set_page_config(page_title="종합 주식 분석 V1.9.4", layout="wide", initial_sidebar_state="expanded") # 버전 업데이트

# --- API 키 로드 ---
NEWS_API_KEY = None; FRED_API_KEY = None; api_keys_loaded = False # 세미콜론 사용
secrets_available = hasattr(st, 'secrets'); sidebar_status = st.sidebar.empty() # 세미콜론 사용
# ... (API 키 로드 상세 로직) ...
if secrets_available:
    try: NEWS_API_KEY = st.secrets.get("NEWS_API_KEY"); FRED_API_KEY = st.secrets.get("FRED_API_KEY"); # 세미콜론 사용
...
if not api_keys_loaded:
    ...
    try:
        dotenv_path = os.path.join(BASE_DIR, '.env'); # 세미콜론 사용
        if os.path.exists(dotenv_path):
            load_dotenv(dotenv_path=dotenv_path); NEWS_API_KEY = os.getenv("NEWS_API_KEY"); FRED_API_KEY = os.getenv("FRED_API_KEY") # 세미콜론 사용
            if NEWS_API_KEY and FRED_API_KEY: api_keys_loaded = True; sidebar_status.success("API 키 로드 완료 (.env)") # 세미콜론 사용
...
with st.sidebar:
    ...
    if page == "📊 종합 분석":
        ...
        st.divider(); st.subheader("⚙️ 예측 세부 설정 (선택)") # 세미콜론 사용
        ...
        st.divider(); st.subheader("💰 보유 정보 입력 (선택)") # 세미콜론 사용
        ...
        st.caption("평단가 입력 시 리스크 트래커 분석 활성화"); st.divider() # 세미콜론 사용
    elif page == "📈 기술 분석":
        ...
        st.caption(f"현재 설정: {bb_window}일 기간, {bb_std:.1f} 표준편차"); st.divider() # 세미콜론 사용

# --- 캐시된 종합 분석 함수 ---
@st.cache_data(ttl=timedelta(hours=1))
def run_cached_analysis(ticker, news_key, fred_key, years, days, num_trend_periods, changepoint_prior_scale):
    try: import stock_analysis as sa # 함수 내 import는 가능하나, 보통 파일 상단에서 함
    ...
    except Exception as e: logging.error(f"analyze_stock 함수 오류: {e}\n{traceback.format_exc()}"); return {"error": f"종합 분석 중 오류 발생: {e}"} # 세미콜론 사용

# --- 메인 화면 로직 ---

# ============== 📊 종합 분석 탭 ==============
if page == "📊 종합 분석":
    st.title("📊 종합 분석 결과"); st.markdown("기업 정보, 재무 추세, 예측, 리스크 트래커 제공."); st.markdown("---") # 세미콜론 사용
            ...
            with st.spinner(f"{ticker_proc} 종합 분석 중..."):
                try:
                    ...
                    if results and isinstance(results, dict) and "error" not in results:
                        # === 상세 결과 표시 시작 (V1.9.3 내용 유지) ===
                        st.header(f"📈 {ticker_proc} 분석 결과 (민감도: {cp_prior:.3f})")
                        # 1. 요약 정보
                        st.subheader("요약 정보"); col1, col2, col3 = st.columns(3); col1.metric("현재가", f"${results.get('current_price', 'N/A')}"); col2.metric("분석 시작일", results.get('analysis_period_start', 'N/A')); col3.metric("분석 종료일", results.get('analysis_period_end', 'N/A')) # 세미콜론 사용
                        # 2. 기본적 분석
                        st.subheader("📊 기업 기본 정보"); fundamentals = results.get('fundamentals') # 세미콜론 사용
                        if fundamentals and isinstance(fundamentals, dict) and fundamentals.get("시가총액", "N/A") != "N/A":
                            colf1, colf2, colf3 = st.columns(3); # 세미콜론 사용
                            with colf1: st.metric("시가총액", fundamentals.get("시가총액", "N/A")); st.metric("PER", fundamentals.get("PER", "N/A")) # 세미콜론 사용
                            with colf2: st.metric("EPS", fundamentals.get("EPS", "N/A")); st.metric("Beta", fundamentals.get("베타", "N/A")) # 세미콜론 사용
                            with colf3: st.metric("배당수익률", fundamentals.get("배당수익률", "N/A")); st.metric("업종", fundamentals.get("업종", "N/A")) # 세미콜론 사용
                            industry = fundamentals.get("산업", "N/A"); summary = fundamentals.get("요약", "N/A") # 세미콜론 사용
                            if industry != "N/A": st.markdown(f"**산업:** {industry}")
                            # --- SyntaxError 수정된 부분 ---
                            # 이 부분의 들여쓰기는 올바르게 수정된 것으로 보입니다.
                            if summary != "N/A":
                                with st.expander("회사 요약 보기"):
                                    st.write(summary) # 다음 줄로 내리고 들여쓰기 (정상)
                            # --- 수정 끝 ---
                            st.caption("Data Source: Yahoo Finance")
                        else: st.warning("기업 기본 정보 로드 실패.")
                        # 3. 주요 재무 추세
                        st.subheader(f"📈 주요 재무 추세 (최근 {periods} 분기)"); tab_titles = ["영업이익률(%)", "ROE(%)", "부채비율", "유동비율"]; tabs = st.tabs(tab_titles); trend_data_map = {"영업이익률(%)": ('operating_margin_trend', 'Op Margin (%)', "{:.2f}%"), "ROE(%)": ('roe_trend', 'ROE (%)', "{:.2f}%"), "부채비율": ('debt_to_equity_trend', 'D/E Ratio', "{:.2f}"), "유동비율": ('current_ratio_trend', 'Current Ratio', "{:.2f}")} # 세미콜론 사용
                        for i, title in enumerate(tab_titles):
                                with tabs[i]:
                                    data_key, col_name, style_format = trend_data_map[title]; trend_data = results.get(data_key) # 세미콜론 사용
                                    if trend_data and isinstance(trend_data, list) and len(trend_data) > 0:
                                        try:
                                            df_trend = pd.DataFrame(trend_data); df_trend['Date'] = pd.to_datetime(df_trend['Date']); df_trend.set_index('Date', inplace=True) # 세미콜론 사용
                                            if col_name in df_trend.columns:
                                                st.line_chart(df_trend[[col_name]]); # 세미콜론 사용
                        ...
                        st.divider()
                        # 4. 기술적 분석 차트 (종합)
                        st.subheader("기술적 분석 차트 (종합)"); stock_chart_fig = results.get('stock_chart_fig') # 세미콜론 사용
                        if stock_chart_fig: st.plotly_chart(stock_chart_fig, use_container_width=True)
                        else: st.warning("주가 차트 생성 실패 (종합)."); st.divider() # 세미콜론 사용
                        # 5. 시장 심리 분석
                        st.subheader("시장 심리 분석"); col_news, col_fng = st.columns([2, 1]) # 세미콜론 사용
                        with col_news:
                            st.markdown("**📰 뉴스 감정 분석**"); news_sentiment = results.get('news_sentiment', ["정보 없음."]) # 세미콜론 사용
                            if isinstance(news_sentiment, list) and len(news_sentiment) > 0:
                                st.info(news_sentiment[0]); # 세미콜론 사용
                                if len(news_sentiment) > 1: with st.expander("뉴스 목록 보기"): [st.write(f"- {line}") for line in news_sentiment[1:]] # 리스트 컴프리헨션으로 여러 위젯 생성 시 주의
                            else: st.write(str(news_sentiment))
                        with col_fng:
                            st.markdown("**😨 공포-탐욕 지수**"); fng_index = results.get('fear_greed_index', "N/A") # 세미콜론 사용
                            if isinstance(fng_index, dict): st.metric("현재 지수", fng_index.get('value', 'N/A'), fng_index.get('classification', ''))
                            else: st.write(fng_index); st.divider() # 세미콜론 사용
                        # 6. Prophet 주가 예측
                        st.subheader("Prophet 주가 예측"); forecast_fig = results.get('forecast_fig'); forecast_data_list = results.get('prophet_forecast') # 세미콜론 사용
                        ...
                        if isinstance(forecast_data_list, list) and len(forecast_data_list) > 0:
                                ...
                                try: df_fcst = pd.DataFrame(forecast_data_list); df_fcst['ds'] = pd.to_datetime(df_fcst['ds']).dt.strftime('%Y-%m-%d'); st.dataframe(df_fcst[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].tail(10).style.format({'yhat': "{:.2f}", 'yhat_lower': "{:.2f}", 'yhat_upper': "{:.2f}"}), use_container_width=True) # 세미콜론 사용
                                except Exception as e: st.error(f"예측 데이터 표시 오류: {e}")
                        cv_plot_path = results.get('cv_plot_path')
                        if cv_plot_path and os.path.exists(cv_plot_path): st.markdown("**📉 교차 검증 결과 (MAPE)**"); st.image(cv_plot_path, caption="MAPE (낮을수록 정확)") # 세미콜론 사용
                        elif cv_plot_path is None and isinstance(forecast_data_list, list): st.caption("교차 검증(CV) 결과 없음."); st.divider() # 세미콜론 사용
                        # 7. 리스크 트래커
                        st.subheader("🚨 리스크 트래커 (예측 기반)"); risk_days, max_loss_pct, max_loss_amt = 0, 0, 0 # 세미콜론 사용
                        if avg_p > 0 and isinstance(forecast_data_list, list) and len(forecast_data_list) > 0:
                                try:
                                    df_pred = pd.DataFrame(forecast_data_list); required_fcst_cols = ['ds', 'yhat_lower'] # 세미콜론 사용
                                    if not all(col in df_pred.columns for col in required_fcst_cols): st.warning("예측 컬럼 부족.")
                                    else:
                                        df_pred['ds'] = pd.to_datetime(df_pred['ds']); df_pred['yhat_lower'] = pd.to_numeric(df_pred['yhat_lower'], 'coerce'); df_pred.dropna(subset=['yhat_lower'], inplace=True) # 세미콜론 사용
                                        if not df_pred.empty:
                                            df_pred['평단가'] = avg_p; df_pred['리스크 여부'] = df_pred['yhat_lower'] < avg_p; df_pred['예상 손실률'] = np.where(df_pred['리스크 여부'], ((df_pred['yhat_lower'] - avg_p) / avg_p) * 100, 0) # 세미콜론 사용
                                            if qty > 0: df_pred['예상 손실액'] = np.where(df_pred['리스크 여부'], (df_pred['yhat_lower'] - avg_p) * qty, 0)
                                            else: df_pred['예상 손실액'] = 0
                                            risk_days = df_pred['리스크 여부'].sum()
                                            if risk_days > 0: max_loss_pct = df_pred['예상 손실률'].min(); max_loss_amt = df_pred['예상 손실액'].min() if qty > 0 else 0 # 세미콜론 사용
                                            st.markdown("##### 리스크 요약"); col_r1, col_r2, col_r3 = st.columns(3); col_r1.metric("⚠️ < 평단가 일수", f"{risk_days}일 / {days}일"); col_r2.metric("📉 Max 손실률", f"{max_loss_pct:.2f}%"); # 세미콜론 사용
                                            ...
                                            st.markdown("##### 평단가 vs 예측 구간 비교"); fig_risk = go.Figure() # 세미콜론 사용
                                            ...
                                            df_risk_periods = df_pred[df_pred['리스크 여부']]; # 세미콜론 사용
                                            ...
                                            fig_risk.update_layout(hovermode="x unified"); st.plotly_chart(fig_risk, use_container_width=True) # 세미콜론 사용
                                            if risk_days > 0:
                                                with st.expander(f"리스크 예측일 상세 데이터 ({risk_days}일)"):
                                                    df_risk_days_display = df_pred[df_pred['리스크 여부']].copy(); df_risk_days_display['ds'] = df_risk_days_display['ds'].dt.strftime('%Y-%m-%d'); cols_show = ['ds', 'yhat_lower', '평단가', '예상 손실률']; # 세미콜론 사용
                                ...
                                except Exception as risk_calc_err: st.error(f"리스크 트래커 오류: {risk_calc_err}"); logging.error(f"Risk tracker error: {traceback.format_exc()}") # 세미콜론 사용
                        elif avg_p <= 0: st.info("⬅️ '평단가' 입력 시 리스크 분석 결과 확인 가능.")
                        else: st.warning("예측 데이터 유효하지 않아 리스크 분석 불가."); st.divider() # 세미콜론 사용
                        # 8. 자동 분석 결과 요약
                        st.subheader("🧐 자동 분석 결과 요약 (참고용)"); summary_points = [] # 세미콜론 사용
                        # (V1.9.3 요약 로직)
                        if isinstance(forecast_data_list, list) and len(forecast_data_list) > 0: # 예측
                            try: start_pred_row = forecast_data_list[0]; end_pred_row = forecast_data_list[-1]; start_pred = pd.to_numeric(start_pred_row.get('yhat'), 'coerce'); end_pred = pd.to_numeric(end_pred_row.get('yhat'), 'coerce'); lower = pd.to_numeric(end_pred_row.get('yhat_lower'), 'coerce'); upper = pd.to_numeric(end_pred_row.get('yhat_upper'), 'coerce'); # 세미콜론 사용
                            except: start_pred, end_pred, lower, upper = None, None, None, None
                            if pd.notna(start_pred) and pd.notna(end_pred): trend_obs = "상승" if end_pred > start_pred else "하락" if end_pred < start_pred else "횡보"; lower_str = f"${lower:.2f}" if pd.notna(lower) else 'N/A'; upper_str = f"${upper:.2f}" if pd.notna(upper) else 'N/A'; summary_points.append(f"- **예측:** 향후 {days}일간 **{trend_obs}** 추세 ({lower_str}~{upper_str}).") # 세미콜론 사용
                            else: summary_points.append("- 예측: 값 유효하지 않음.")
                        news_res = results.get('news_sentiment'); fng_res = results.get('fear_greed_index') # 세미콜론 사용
                        if isinstance(news_res, list) and len(news_res) > 0 and ":" in news_res[0]: # 뉴스
                            try: score_part = news_res[0].split(":")[-1].strip(); avg_score = float(score_part); sentiment_desc = "긍정적" if avg_score > 0.05 else "부정적" if avg_score < -0.05 else "중립적"; summary_points.append(f"- **뉴스:** 평균 감성 {avg_score:.2f}, **{sentiment_desc}** 분위기.") # 세미콜론 사용
                            except Exception as e: logging.warning(f"뉴스 요약 오류: {e}"); summary_points.append("- 뉴스: 요약 오류.") # 세미콜론 사용
                        if isinstance(fng_res, dict): summary_points.append(f"- **시장 심리:** 공포-탐욕 {fng_res.get('value', 'N/A')} (**{fng_res.get('classification', 'N/A')}**).")
                        if fundamentals and isinstance(fundamentals, dict): # 기본 정보
                                per = fundamentals.get("PER", "N/A"); sector = fundamentals.get("업종", "N/A"); parts = []; # 세미콜론 사용
                                if per != "N/A": parts.append(f"PER {per}")
                                if sector != "N/A": parts.append(f"업종 '{sector}'")
                                if parts: summary_points.append(f"- **기본 정보:** {', '.join(parts)}.")
                        trend_parts = [] # 재무 추세
                        try:
                            op_margin_trend = results.get('operating_margin_trend'); roe_trend = results.get('roe_trend'); debt_trend = results.get('debt_to_equity_trend'); current_trend = results.get('current_ratio_trend') # 세미콜론 사용
                        ...
                        except Exception as e: logging.warning(f"재무 추세 요약 오류: {e}"); summary_points.append("- 최근 재무: 요약 오류.") # 세미콜론 사용
                        if avg_p > 0 and isinstance(forecast_data_list, list) and len(forecast_data_list) > 0: # 리스크
                            if risk_days > 0: summary_points.append(f"- **리스크:** {days}일 중 **{risk_days}일** 평단가 하회 가능성 (Max 손실률: **{max_loss_pct:.2f}%**).")
                            else: summary_points.append(f"- **리스크:** 평단가(${avg_p:.2f}) 하회 가능성 낮음.")
                        elif avg_p > 0: summary_points.append("- 리스크: 평단가 입력됨, 분석 불가.")
                        if summary_points: st.markdown("\n".join(summary_points)); st.caption("⚠️ **주의:** 투자 조언 아님.") # 세미콜론 사용
...
# ============== 📈 기술 분석 탭 (MultiIndex Fix + Auto Interpretation) ==============
    ...
    today = datetime.now().date(); default_start_date = today - relativedelta(months=3) # 세미콜론 사용
    ...
    if analyze_button_tech:
        if not ticker_tech: st.warning("종목 티커를 입력해주세요.")
            ...
            with st.spinner(f"{ticker_processed_tech} 데이터 로딩 중..."):
                try: # yfinance 다운로드
                    period_days = (end_date - start_date).days; fetch_start_date = start_date # 세미콜론 사용
                    if interval == '1m' and period_days > 7: st.warning("1분봉 최대 7일 조회. 시작일 조정."); fetch_start_date = end_date - timedelta(days=7) # 세미콜론 사용
                    elif interval in ['5m', '15m', '30m'] and period_days > 60: st.warning(f"{interval_display} 최대 60일 조회. 시작일 조정."); fetch_start_date = end_date - timedelta(days=60) # 세미콜론 사용
            ...
            if not df_tech.empty:
                ...
                if isinstance(df_tech.columns, pd.MultiIndex):
                    logging.info("MultiIndex 컬럼 감지됨. Flattening 시도..."); original_columns = df_tech.columns # 세미콜론 사용
                    df_tech.columns = df_tech.columns.get_level_values(0); logging.info(f"컬럼 변환 완료: {original_columns.tolist()} -> {df_tech.columns.tolist()}") # 세미콜론 사용
                    df_tech = df_tech.loc[:,~df_tech.columns.duplicated()]; logging.info(f"중복 제거 후 컬럼: {df_tech.columns.tolist()}") # 세미콜론 사용
                # --- 필수 컬럼 검증 ---
                required_cols = ['Open', 'High', 'Low', 'Close', 'Volume']
                missing_cols = [col for col in required_cols if col not in df_tech.columns]
                if missing_cols:
                    st.error(f"❌ 데이터에 필수 컬럼 누락: {missing_cols}. 실제 컬럼: {df_tech.columns.tolist()}"); st.dataframe(df_tech.head()) # 세미콜론 사용
            ...
            if analysis_successful:
                with st.spinner("기술적 지표 계산 및 차트 생성 중..."):
                    try:
                        df_processed = df_tech.dropna(subset=required_cols).copy() # 이제 KeyError 발생 안 함
                        if df_processed.empty: st.warning("데이터 정제 후 남은 데이터가 없습니다.")
                            ...
                            if not df_calculated.empty:
                                latest_row = df_calculated.iloc[-1]; signal_messages = [] # 세미콜론 사용
                                ...
                                if signal_messages: [st.info(msg) for msg in signal_messages] # 리스트 컴프리헨션 사용 주의
...
# --- 앱 정보 ---
st.sidebar.markdown("---")
st.sidebar.info("종합 주식 분석 툴 V1.9.4 | 정보 제공 목적 (투자 조언 아님)")