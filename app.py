# -*- coding: utf-8 -*-
# Combined app.py V1.9 - Enhanced Error Handling in Technical Analysis Tab
...
def calculate_vwap(df):
    ...
    if not all(col in df.columns for col in required_cols):
        # 함수 호출 전에 컬럼 체크하므로, 여기서 에러 발생하면 로직 오류 가능성 있음
        missing = [col for col in required_cols if col not in df.columns]
        raise ValueError(f"VWAP 계산 내부 오류: 컬럼 부족 ({missing})")
    if df['Volume'].isnull().all() or df['Volume'].sum() == 0: # Volume이 없거나 모두 0인 경우
        df['VWAP'] = np.nan
        logging.warning(f"Ticker {df.attrs.get('ticker', '')}: VWAP 계산 불가 (거래량 데이터 부족 또는 0)")
    else:
        # Volume NaN 처리 (ffill은 가격 왜곡 가능성, 0으로 채우기 고려)
        df['Volume'].fillna(0, inplace=True) # 거래 없는 날 NaN을 0으로 간주
...
def calculate_bollinger_bands(df, window=20, num_std=2):
    ...
    if required_col not in df.columns or df[required_col].isnull().all():
        raise ValueError(f"볼린저 밴드 계산 필요 컬럼 '{required_col}' 없거나 데이터 없음.")
    if len(df.dropna(subset=[required_col])) < window: # NaN 제외하고 window 개수 확인
        st.warning(f"볼린저 밴드 계산 위한 유효 데이터({len(df.dropna(subset=[required_col]))}개)가 기간({window}개)보다 부족.")
        df['MA20'] = np.nan; df['Upper'] = np.nan; df['Lower'] = np.nan
    else:
        df['MA20'] = df[required_col].rolling(window=window, min_periods=window).mean() # min_periods 추가
...
def plot_technical_chart(df, ticker):
    ...
    if not valid_price_df.empty:
        ...
        else: st.caption("기간 내 가격 변동 없어 피보나치 레벨 미표시.")
...
# --- Streamlit 페이지 설정 ---
st.set_page_config(page_title="종합 주식 분석 V1.9", layout="wide", initial_sidebar_state="expanded") # 버전 업데이트
            ...
            with st.spinner(f"{ticker_proc} 종합 분석 중..."):
                try:
                    ...
                    if results and isinstance(results, dict) and "error" not in results:
                        # === 상세 결과 표시 시작 (V1.8 내용 유지) ===
                        st.header(f"📈 {ticker_proc} 분석 결과 (민감도: {cp_prior:.3f})")
                        # 1. 요약 정보 ... (이하 상세 결과 표시 로직 유지)
                             ...
                             with tabs[i]:
                                 ...
                                 if trend_data and isinstance(trend_data, list) and len(trend_data) > 0:
                                     try:
                                         ...
                                         if col_name in df_trend.columns: st.line_chart(df_trend[[col_name]]); # Expander는 생략 가능
                        ...
                        if isinstance(forecast_data_list, list) and len(forecast_data_list) > 0:
                             st.markdown("**📊 예측 데이터 (최근 10일)**"); # Expander 생략 가능
                             try:
                             ...
                             try:
                                 ...
                                 if not all(col in df_pred.columns for col in required_fcst_cols): st.warning("예측 컬럼 부족.")
                                     ...
                                     if not df_pred.empty:
                                         ...
                                         st.markdown("##### 평단가 vs 예측 구간 비교"); fig_risk = go.Figure() # 차트 표시 로직 간소화 가능
                        ...
                        if isinstance(forecast_data_list, list) and len(forecast_data_list) > 0: # 예측
                            try: # ... (V1.8 요약 로직) ...
                                start_pred_row = forecast_data_list[0]; end_pred_row = forecast_data_list[-1]; start_pred = pd.to_numeric(start_pred_row.get('yhat'), 'coerce'); end_pred = pd.to_numeric(end_pred_row.get('yhat'), 'coerce'); lower = pd.to_numeric(end_pred_row.get('yhat_lower'), 'coerce'); upper = pd.to_numeric(end_pred_row.get('yhat_upper'), 'coerce')
                        ...
                        if isinstance(results.get('news_sentiment'), list) and len(results['news_sentiment']) > 0 and ":" in results['news_sentiment'][0]: # 뉴스
                            try: # ... (V1.8 요약 로직) ...
                                score_part = results['news_sentiment'][0].split(":")[-1].strip(); avg_score = float(score_part); sentiment_desc = "긍정적" if avg_score > 0.05 else "부정적" if avg_score < -0.05 else "중립적"; summary_points.append(f"- **뉴스:** 평균 감성 {avg_score:.2f}, **{sentiment_desc}** 분위기.")
...
# ============== 📈 기술 분석 탭 (Error Handling 수정) ==============
    ...
    if analyze_button_tech:
        if not ticker_tech: st.warning("종목 티커를 입력해주세요.")
            ...
            df_tech = pd.DataFrame() # df_tech 초기화
                    ...
                    with st.spinner("기술적 지표 계산 및 차트 생성 중..."):
                        try:
                            ...
                            if df_tech.empty:
                                ...
                                display_cols = [col for col in display_cols if col in df_tech.columns] # 존재하는 컬럼만
...
# --- 앱 정보 ---
st.sidebar.markdown("---")
st.sidebar.info("종합 주식 분석 툴 V1.9 | 정보 제공 목적 (투자 조언 아님)")