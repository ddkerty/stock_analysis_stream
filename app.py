# app.py (뉴스 목록 표시 SyntaxError 최종 수정)
        ...
        with st.spinner(f"{ticker_processed} 종목 분석 중..."):
            try:
                ...
                if results and isinstance(results, dict):
                    ...
                    st.dataframe(df_fcst[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].tail(10).style.format({'yhat': "{:.2f}", 'yhat_lower': "{:.2f}", 'yhat_upper': "{:.2f}"}), use_container_width=True) # 수정된 포맷팅 적용
                    ...
                    if avg_price > 0 and isinstance(forecast_data_list, list):
                        ...
                        try:
                            ...
                            else: st.success(f"향후 {forecast_days}일 동안 예측 하한선이 평단가(${avg_price:.2f})보다 낮아질 가능성은 없는 것으로 예측됨.")
                    ...
                    if roe_trend_data and isinstance(roe_trend_data, list) and len(roe_trend_data) >= 2: # 최소 2개 분기 데이터 필요
                        try:
                            first_roe = pd.to_numeric(roe_trend_data[0]['ROE (%)'], errors='coerce')
                            last_roe = pd.to_numeric(roe_trend_data[-1]['ROE (%)'], errors='coerce')
