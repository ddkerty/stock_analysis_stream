# stock_analysis.py (최종 오류 수정 및 검토 버전)
...
def get_operating_margin_trend(ticker, num_periods=4):
    ...
    df[revenue_col] = df[revenue_col].replace(0, np.nan); df.dropna(subset=[revenue_col, op_income_col], inplace=True);
...
def get_debt_to_equity_trend(ticker, num_periods=4):
    ...
    td_col = find_financial_statement_item(qbs.index, ['Total', 'Debt'], ['Total Debt']); sd_col = find_financial_statement_item(qbs.index, ['Current', 'Debt'], ['Current Debt']); ld_col = find_financial_statement_item(qbs.index, ['Long', 'Term', 'Debt'], ['Long Term Debt'])
...
def get_current_ratio_trend(ticker, num_periods=4):
    ...
    if qbs.empty: logging.warning(f"{ticker}: 분기 대차대조표 없음."); return None
    ca_col = find_financial_statement_item(qbs.index, ['Total', 'Current', 'Assets'], ['Total Current Assets']); cl_col = find_financial_statement_item(qbs.index, ['Total', 'Current', 'Liabilities'], ['Total Current Liabilities'])
...
def run_prophet_forecast(ticker, start_date, end_date=None, forecast_days=30, fred_key=None):
    ...
    if not df_macro.empty:
        try:
            ...
            logging.info(f"{ticker}: 매크로 ffill/bfill 완료.")
             ...
             logging.error(f"{ticker}: 데이터 병합 오류: {merge_err}"); df_merged = df_stock_processed; logging.warning(f"{ticker}: 주가만 사용.")
    else: logging.warning(f"{ticker}: 매크로 없어 주가만 사용."); df_merged = df_stock_processed
    ...
    logging.info(f"Prophet 학습 데이터 준비: {len(df_merged)}"); os.makedirs(DATA_FOLDER, exist_ok=True); data_csv = os.path.join(DATA_FOLDER, f"{ticker}_prophet.csv")
    try: df_merged.to_csv(data_csv, index=False); logging.info(f"학습 데이터 저장: {data_csv}")
    ...
    if not df_macro.empty:
        ...
        if macro_cols_available: [m.add_regressor(col) for col in macro_cols_available]; regressors.extend(macro_cols_available); logging.info(f"{ticker}: Regressors 추가: {macro_cols_available}")
    ...
    try:
        ...
        try:
            ...
            if len(df_prophet) > int(initial.split()[0]) + int(horizon.split()[0]) + int(period.split()[0]):
                 logging.info(f"Prophet CV 시작..."); df_cv = cross_validation(m, initial=initial, period=period, horizon=horizon, parallel=None)
                 logging.info("CV 완료."); df_p = performance_metrics(df_cv); logging.info(f"Prophet 성능:\n{df_p.head().to_string()}")
                 fig_cv = plot_cross_validation_metric(df_cv, metric='mape'); plt.title(f'{ticker} CV MAPE'); cv_path = os.path.join(FORECAST_FOLDER, f"{ticker}_cv_mape_plot.png"); fig_cv.savefig(cv_path); plt.close(fig_cv); logging.info(f"CV MAPE 차트 저장: {cv_path}")
            else: logging.warning(f"{ticker}: 데이터 기간 부족 CV 건너뜀 ({len(df_prophet)} 일)."); cv_path = None
        ...
        if regressors: # 미래 Regressor 처리
            ...
            temp_m = df_merged.copy(); temp_m['ds'] = pd.to_datetime(temp_m['Date'])
...
def analyze_stock(ticker, news_key, fred_key, analysis_period_years=2, forecast_days=30, num_trend_periods=4):
    ...
    if stock_data_valid: output_results['current_price'] = f"{df_stock_full['Close'].iloc[-1]:.2f}" if not df_stock_full['Close'].empty else "N/A"; output_results['data_points'] = len(df_stock_full)
    ...
    if stock_data_valid and output_results['data_points'] > 30 : # 조건 변경: 주가 데이터 유효성 직접 체크
        ...
        else: output_results['prophet_forecast'] = "예측 실행 오류"; output_results['forecast_fig'] = None; output_results['cv_plot_path'] = None; logging.error(f"{ticker}: run_prophet_forecast 비정상 반환.")
    else: msg = f"데이터 부족({output_results['data_points']})" if output_results['data_points'] <= 30 else "주가 정보 없음"; output_results['prophet_forecast'] = f"{msg} 예측 불가"; output_results['forecast_fig'] = None; output_results['cv_plot_path'] = None; logging.warning(f"{msg} Prophet 예측 건너뜀.")
